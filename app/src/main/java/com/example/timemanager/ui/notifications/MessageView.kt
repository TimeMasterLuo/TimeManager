...
import com.example.timemanager.adapter.messageAdapter.clockMessageAdapter
import com.example.timemanager.adapter.messageAdapter.friendClockMessage
...
class MessageView : AppCompatActivity() {
    ...
    private var friendClockMessageList: MutableList<friendClockMessage> = arrayListOf()
    var addFriendMessageAdapter: addMessageAdapter? = null
    var friendClockMessageAdapter: clockMessageAdapter? = null
    ...
    private fun getAddClockMessage()
    {
        val globalData: TimeManager = application as TimeManager
        val url = "http://59.78.38.19:8080/getFriendClock"
        //定义发送的json数据，JSONObject初始化的其他方式还需自行探索
        //val params = JSONArray("""[{"Username":${globalData.username}}]""")
        val params = JSONObject("""{"username":${globalData.username}}""")
        //发送请求
        ...
        val jsonObjectRequest = JsonObjectRequest(
            ...
            { response ->
                ...
                if(response.getJSONArray("data").length() == 0) return@JsonObjectRequest
                    ...
                    {
                        val myresponse = response.getJSONArray("data").getJSONObject(i)
                        val message: friendClockMessage =
                            friendClockMessage()
                        message.id = myresponse.getString("id")
                        message.toId = myresponse.getString("toId")
                        message.sender = myresponse.getString("fromName")
                        message.receiver = myresponse.getString("toName")
                        message.status = "待处理" //  这个是后端数据库里的acceptStatus
                        message.type = "好友闹钟"
                        message.coins = myresponse.getInt("coins")
                        message.score = myresponse.getInt("score")
                        message.task = myresponse.getString("task")
                        message.updateTime = myresponse.getString("updateTime")
                        message.recordTime = myresponse.getString("recordTime")
                        message.note = myresponse.getString("note")
                        message.clockStatus = myresponse.getString("status")
                        friendClockMessageList.add(i, message)
                    }
                    friendClockMessageAdapter = clockMessageAdapter(this, friendClockMessageList)
                    taskClockList.adapter = friendClockMessageAdapter
                }
